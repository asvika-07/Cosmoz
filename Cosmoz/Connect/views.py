...
from Connect.models import FriendRequest, FriendList
...
def ConnectView(request):
    ...
    if user.is_authenticated:
        return render(request, 'friend/Connect.html')
    else:
...
def friends_list_view(request,user_id):
    ...
    user = request.user
    if user.is_authenticated:
        user_id = get_object_or_404(User, email=user_id)
        #user_id=get_object_or_404(User, RegistrationID=reg_id)
        if user_id:
            try:
...
def friend_requests(request,user_id):
    ...
    if user.is_authenticated:
        ...
        account =get_object_or_404(User, email=user_id)
        if account == user:
...
def remove_friend(request, *args, **kwargs):
    ...
    payload = {}
    if request.method == "POST" and user.is_authenticated:
        user_id = request.POST.get("receiver_email")
        print(user_id)
        if user_id:
            try:
